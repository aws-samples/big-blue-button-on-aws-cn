...
main() {
  ...
  PACKAGE_REPOSITORY=aws-solutions-assets.s3.cn-north-1.amazonaws.com.cn/bigbluebutton
...
check_version() {
  ...
  if ! wget -qS --spider "http://$PACKAGE_REPOSITORY/$1/dists/bigbluebutton-$DISTRO/Release.gpg" > /dev/null 2>&1; then
  ...
  if ! apt-key list | grep -q "BigBlueButton apt-get"; then
    wget http://$PACKAGE_REPOSITORY/repo/bigbluebutton.asc -O- | apt-key add -
  fi
  ...
  echo "deb [trusted=yes] http://$PACKAGE_REPOSITORY/$VERSION bigbluebutton-$DISTRO main" > /etc/apt/sources.list.d/bigbluebutton.list
}
